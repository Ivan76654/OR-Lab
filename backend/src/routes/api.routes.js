...
apiRouter.get('/data', (req, res, next) => {
	(async () => {
		try {
			let playerSql =
				'SELECT DISTINCT player.* FROM league NATURAL JOIN team NATURAL JOIN player ORDER BY player.player_id;';
			let teamSql =
				'SELECT DISTINCT team.* FROM league NATURAL JOIN team NATURAL JOIN player ORDER BY team.team_id;';
			let leagueSql =
				'SELECT DISTINCT league.* FROM league NATURAL JOIN team NATURAL JOIN player ORDER BY league.league_id;';
					...
					.map((player) => {
						...
						const schemaPerson = {
							'@context': {
								...
								player_first_name: 'givenName',
								player_last_name: 'familyName',
			...
			for (let i = 0; i < formattedLeagueResult.length; i++) {
				...
				const leagueTeams = formattedTeamResult
					.filter(
						(team) => currentLeague.league_id === team.league_id
					)
					.map((team) => {
...
apiRouter.get('/leagues', (req, res, next) => {
	(async () => {
		try {
			const sqlQuery =
				'SELECT league_id, league_rank, league_name FROM league ORDER BY league_id;';
			const params = [];
...
apiRouter.get('/teams', (req, res, next) => {
	(async () => {
		try {
			const sqlQuery =
				'SELECT team_id, team_name, founded FROM team ORDER BY team_id;';
			const params = [];
...
apiRouter.get('/players', (req, res, next) => {
	(async () => {
		try {
			const sqlQuery =
				'SELECT player_id, player_first_name, player_last_name, date_of_birth, elo_rank FROM player ORDER BY player_id;';
			const params = [];

			const result = (await query(sqlQuery, params)).rows;

			const resultWithPersonSchema = result.map((player) => {
				const schemaPerson = {
					'@context': {
						'@vocab': 'http://schema.org/',
						player_first_name: 'givenName',
			...
			res.json({
				...
				response: resultWithPersonSchema
			});
		} catch (err) {
			next({
				status: 500,
...
apiRouter.post('/players', (req, res, next) => {
	(async () => {
		try {
			...
			sqlQuery =
				'INSERT INTO player (player_first_name, player_last_name, date_of_birth, elo_rank, team_id) VALUES ($1, $2, $3, $4, $5) RETURNING *;';
			params = [
				body.player_first_name,
				body.player_last_name,
				body.date_of_birth,
...
apiRouter.get('/players/:playerId', (req, res, next) => {
	(async () => {
		try {
			...
			const sqlQuery =
				'SELECT player_id, player_first_name, player_last_name, date_of_birth, elo_rank FROM player WHERE player_id = $1;';
